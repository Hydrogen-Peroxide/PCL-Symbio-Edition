...
public partial class FormMain
{
    ...
    private void ShowUpdateLog()
    {
        ModBase.RunInNewThread(() =>
        {
            ...
            if (ModMain.MyMsgBoxMarkdown(changelog,
                ...
                2) ModBase.OpenWebsite("https://github.com/PCL-Community/PCL2-CE/releases/latest");
        }, "UpdateLog Output");
    }
    ...
    public FormMain()
    {
        ...
        // 加载 UI
        InitializeComponent();
        SourceInitialized += FormMain_SourceInitialized;
        Opacity = 0d;
        try
        {
    ...
    private void FormMain_SourceInitialized(object? sender, EventArgs e)
    {
        // 硬件加速
    ...
    ///     通过点击导航栏改变页面。
    /// </summary>
    private void BtnTitleSelect_PreviewClick(object sender, ModBase.RouteEventArgs e)
    {
        if (isChangingPage)
            return;
        var btn = (MyRadioButton)sender;
        var pageType = (PageType)int.Parse(btn.Tag.ToString());
        ModBase.Log("[Control] 导航按钮点击：" + btn.Text + " → " + pageType);
        PageChangeActual(pageType, PageSubType.Default);
    }

    private void BtnTitleSelect_Click(MyRadioButton sender, bool raiseByMouse)
    {
        if (isChangingPage || !isWindowLoadFinished)
            return;
    ...
    ///     直接切换到基岩版页面（绕过导航栏逻辑）。
    /// </summary>
    public void PageChangeToBedrock()
    {
        try
        {
            isChangingPage = true;
            // 反向遍历取消所有导航栏按钮的选中状态
            // 避免已选中按钮（如"启动"）因 MyRadioButton 的 Radiobox_MouseUp 中
            // "if (Checked) return" 逻辑而无法再次点击
            var children = PanTitleSelect.Children;
            for (var i = children.Count - 1; i >= 0; i--)
            {
                if (children[i] is MyRadioButton btn)
                    btn.SetChecked(false, true, true);
            }
            isChangingPage = false;
            PageChangeActual(PageType.Bedrock, PageSubType.Default);
        }
        catch (Exception ex)
        {
            isChangingPage = false;
            ModBase.Log(
                ...
                userSummary: "切换至基岩版页面失败：" + ex.Message);
        }
    }
    ...
    ///     切换现有页面的实际方法。
    /// </summary>
    internal void PageChangeActual(PageStackData stack, PageSubType subType)
    {
        ...
        try
        {
            ...
            switch (stack.page)
            {
                ...
                case PageType.Bedrock: // 基岩版
                    {
                        if (ModMain.frmBedrockLeft is null)
                            ModMain.frmBedrockLeft = new PageBedrockLeft();
                        if (ModMain.frmBedrockRight is null)
